...
public final class HudExpressionEngine {
    ...
    public static Object eval(String expression, Resolver resolver) {
        ...
        if (split != null) {
            boolean condition = asBoolean(eval(split.condition, resolver));
            return condition ? eval(split.whenTrue, resolver) : eval(split.whenFalse, resolver);
        }

        Parser parser = new Parser(raw);
        Node node = parser.parseExpression();
        return node.evaluate(resolver);
    }
    ...
    public static String evalTemplate(String rawValue, Resolver resolver) {
        ...
        while (index < normalized.length()) {
            ...
            output.append(normalized, index, open);
        ...
        if (output.length() == 0 && normalized.indexOf('{') < 0) {
            return normalized;
        }
